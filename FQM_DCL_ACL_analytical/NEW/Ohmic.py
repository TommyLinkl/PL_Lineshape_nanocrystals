...
corr_file = "corr_Ohmic_T=2.dat"
spectrum_file = "I_Ohmic_T=2.dat"
...
def Int_FQM_Im(time): 
    ...
    y_range = np.zeros(0)
    for w in w_range:
        y_range = np.append(y_range, FQM_Im(w, time))
    return simpson(y_range, w_range)

def DCL_Re(omega, time):
    return J(omega) * (np.cos(omega*time) - 1) / BETA
...
def DCL_Im(omega, time):
    return -0.5 * J(omega) * omega**2 * time
...
def Int_ACL_Re(time): 
    ...
    y_range = np.zeros(0)
    for w in w_range:
        y_range = np.append(y_range, ACL_Re(w, time))
    return simpson(y_range, w_range)
