...
def spectrum(E_shift, t_range, F_Re, F_Im, E_range):
    ...
    for E in E_range:
        w = E/(JTOEV * HBAR)      # omega in Hz
        integrand = np.zeros(0)
        for i in range(len(t_range)):
            integrand = np.append(integrand, np.cos((w-E_shift/HBAR) * t_range[i]) * F_Re[i] - np.sin((w-E_shift/HBAR) * t_range[i]) * F_Im[i])
        I_w = simpson(integrand, x=t_range)
        I = np.append(I, I_w)
...
def calc_spectrum_classical_MC(T, t_range, w_range, cutoff, slope, exc_E, phonon_filename, coupling_filename, I_file, FWHM_file):
    ...
    phonons = phonons[6:]
    V = coupling[6:, 0]
    delta_range_original = (V) / phonons**2
    
    delta_file = open("delta.dat", "w")
    delta_file.write("# phMode         phFreq        naCouplingDelta          fittedDelta \n")
    
    A = 4427036.251
    B = 3.728
    C = -0.401
    phonons = phonons / (1e12 * (2*PI))
    delta_range = np.sqrt(A * phonons**(B-3) * np.exp(phonons/C) * 2 / PI)    
    phonons = phonons * 1e12 * (2*PI)
    for i in range(len(phonons)):
        delta_file.write("%f       %f       %.30f         %.30f \n" % (i, phonons[i], delta_range_original[i], delta_range[i]))
    print("DONE reading")
