...
fig, axs = plt.subplots(len(T_range), 2, figsize=(12, 4*len(T_range)))
...
for i in range(len(T_range)): 
    ...
    axs[i,0].grid()
    axs[i,0].legend()
    axs[i,0].set(title="Dephasing function at T=%d"%T, xlabel="time [s]", ylabel=r"$\langle F(t) \rangle$ [unitless]", xlim=[0, 1e-12])
    ...
    axs[i,1].grid()
    axs[i,1].legend()
    axs[i,1].set(xlabel=r"Energy [eV]", ylabel=r"$I(\omega)$", xlim=[2.0,2.5])

'''
axs[0,2].set(xlim=[2.175, 2.20])
axs[1,2].set(xlim=[2.175, 2.20])
axs[2,2].set(xlim=[2.15, 2.22])
axs[3,2].set(xlim=[2.14, 2.24])
'''

fig.tight_layout()
fig.savefig("allPhonon_QM_E17.png")
