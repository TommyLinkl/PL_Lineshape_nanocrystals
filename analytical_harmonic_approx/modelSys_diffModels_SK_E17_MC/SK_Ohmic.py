...
def J(omega):
    return 2 / PI * omega**(-2) * np.exp(-omega)
...
fig.suptitle("Convergence for MC's equivalent of SK's Ohmic  \n")
...
for i in range(4):
    ...
    axs[i].grid()
    axs[i].set(xlabel="Time", ylabel=r"$\langle F(t) \rangle _{Re}$", yscale="log", title="T=%.0f" % T)
    ...
    for N in np.array([1000, 10000]): 
        ...
        axs[i].plot(t_range, c_FQM_Re, label="NB=%.0f" % N)
    axs[i].legend()

fig.tight_layout()
fig.savefig("Skinner_Ohmic.png")
