...
t_range = np.append(t_range, np.arange(1e-9, 1e-5+dt3, dt3))
...
F_t_Re_E17_erf = multiply_erf(cutoff, slope, corr_t-np.mean(corr_t[-1000:-1]), dt)
F_t_Im_E17_erf = multiply_erf(cutoff, slope, corr_t-np.mean(corr_t[-1000:-1]), dt)
F_t_Re_MC_erf = multiply_erf(cutoff, slope, corr_t-np.mean(corr_t[-1000:-1]), dt)
F_t_Im_MC_erf = multiply_erf(cutoff, slope, corr_t-np.mean(corr_t[-1000:-1]), dt)
...
delta_E_E17 = excitonic_e*AUTOJ - reorg_E(delta_range, w)
delta_E_MC = excitonic_e*AUTOJ
(spectrum_w_E17, spectrum_I_E17) = spectrum(delta_E_E17/HBAR, t_range, F_t_Re_E17_erf, F_t_Im_E17_erf, domega)
print("DONE spectrum_E17")
(spectrum_w_MC, spectrum_I_MC) = spectrum(delta_E_MC/HBAR, t_range, F_t_Re_MC_erf, F_t_Im_MC_erf, domega)
...
fig.suptitle(r"Long time plot, 3nmCdSe at T=1K")
...
# axs[0,2].plot(spectrum_w_MC, spectrum_I_MC, label=r"FT of $\langle F(t) \rangle _{MC} \cdot erf$")

# axs[2,2].set(xlabel="Frequency", title="E17 Spectrum")
# axs[2,2].plot(spectrum_w_E17, spectrum_I_E17, label=r"FT of $\langle F(t) \rangle _{E17} \cdot erf$")
