...
x2 = np.array([float(line.strip().split()[0]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_2MLCdS/results/OLD_gaussian500fs_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
...
x4 = np.array([float(line.strip().split()[0]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_2MLCdS/results/NEW_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
...
axs[0].plot(4162-x3, y3, "g", linewidth=1, label="Utzat, CdSe/CdS/ZnS") # 4K
axs[0].set(xlim=(2000,2200), ylim=(0, 1), xlabel="Energy [meV]", ylabel="Abs", title="Single-NC line shape at low T (5K)")
axs[0].legend(loc="upper right", prop={"size": 10})
axins = inset_axes(axs[0], width="100%", height="100%", bbox_to_anchor=(.5, .35, .48, .45), bbox_transform=axs[0].transAxes)
...
axs[1].plot(4052-x3, y3, "g", linewidth=1, label="Utzat, CdSe/CdS/ZnS") # 290K
axs[1].set(xlim=(1900,2200), ylim=(0, 1), xlabel="Energy [meV]", ylabel="Abs", title="Single-NC line shape at high T (300K)")
