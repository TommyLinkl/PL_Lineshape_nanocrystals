...
axs[0].plot(3989-x3, y3, ":b", linewidth=1, label="Meijerink JPCC") # 295K
axs[0].set(xlim=(1800,2100), ylim=(0, 1), xlabel="Energy [meV]", ylabel="Abs", title="QD ensemble line shape at high T (300K)")
axs[0].legend(loc="upper right", prop={"size": 10})


x1 = np.array([float(line.strip().split()[0]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_4MLCdS/results/NEW_gaussian30fs_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
y1 = np.array([float(line.strip().split()[1]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_4MLCdS/results/NEW_gaussian30fs_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
x2 = np.array([float(line.strip().split()[0]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_4MLCdS/results/OLD_gaussian30fs_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
y2 = np.array([float(line.strip().split()[1]) for line in \
    open("../../CALCS/coreShell_3.0nmCdSe_4MLCdS/results/OLD_gaussian30fs_Spectrum_QM_E17_T=5.dat", 'r') if line[0]!="#"])
x3 = np.array([float(line.strip().split()[0]) for line in \
...
axs[1].plot(4045-x3, y3, ":b", linewidth=1, label="Meijerink JPCC") # 4K
axs[1].set(xlim=(1800,2100), ylim=(0, 1), xlabel="Energy [meV]", ylabel="Abs", title="QD ensemble line shape at low T (5K)")
