...
# Data
phonons = np.array([float(line.strip().split()[1]) for line in open("w.dat", 'r') if line[0]!="#"])
OLD_V = np.array([float(line.strip().split()[3]) for line in open("Vklq-diabatic_OLD.dat", 'r') if line[0]!="#"])
NEW_V = np.array([float(line.strip().split()[3]) for line in open("Vklq-diabatic_NEW.dat", 'r') if line[0]!="#"])
# NEW_V = np.array([float(line.strip().split()[3]) for line in open("../scale_coupling_coreShell/nonuniform_scaling_trial7/results/Vklq_case77.dat", 'r') if line[0]!="#"])
phonons = phonons[6:]
OLD_V = OLD_V[6:]
NEW_V = NEW_V[6:]
