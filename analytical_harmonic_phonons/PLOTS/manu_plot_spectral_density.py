...
# Data
phonons = np.array([float(line.strip().split()[1]) for line in open("../CALCS/coreShell_3.0nmCdSe_3MLCdS/w.dat", 'r') if line[0]!="#"])
# OLD_V = np.array([float(line.strip().split()[3]) for line in open("../CALCS/coreShell_3.0nmCdSe_3MLCdS/OLD_Vklq-diabatic.dat", 'r') if line[0]!="#"])
NEW_V = np.array([float(line.strip().split()[3]) for line in open("../CALCS/coreShell_3.0nmCdSe_3MLCdS/NEW_Vklq-diabatic.dat", 'r') if line[0]!="#"])
phonons = phonons[6:]
# OLD_V = OLD_V[6:]
NEW_V = NEW_V[6:]
...
# fig.suptitle("3nm CdSe/3ML CdS")

# axs.plot(x_OLD, y_OLD, "-r", linewidth=1, label="Old code")
axs.plot(x_NEW, y_NEW, "-k", linewidth=1, label="3nm CdSe/3ML CdS")
axs.get_yaxis().set_ticks([0])
axs.set(xlim=(0,11.4), xlabel="Frequency [THz]", ylabel=r"Spectral density $J\left( \omega \right)$")
# , title=r"$J_{Dipti}\left( \omega \right) = \sum _\alpha \left( \frac {V^\alpha}{\omega_\alpha} \right)^2 \delta \left( \omega - \omega_\alpha \right)$")
axs.set_ylim(bottom=0, top=None)
# axs.legend(loc="upper left")
axs.text(0.2, 370000, "3nm CdSe/3ML CdS")

axins = inset_axes(axs, width="100%", height="100%", bbox_to_anchor=(.15, .5, .35, .3), bbox_transform=axs.transAxes)
axins.plot(x_inset, y_inset, "-k", linewidth=0.8)
