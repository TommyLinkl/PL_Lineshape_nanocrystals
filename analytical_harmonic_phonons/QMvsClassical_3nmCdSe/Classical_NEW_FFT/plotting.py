...
T_range = np.array([1, 2, 10, 20, 200, 300])

fig, axs = plt.subplots(len(T_range), 1, figsize=(18, 4*len(T_range)))

for i in range(len(T_range)): 
    T = T_range[i]
    F_file = "NEW_F_CL_T=%d.dat" % T
    I_file = "NEW_Spectrum_CL_T=%d.dat" % T
    ...
    axs[i].plot(E_range, I_Gaussian, label="Static limit")
    axs[i].grid()
    axs[i].legend()
    axs[i].set(title="T=%d"%T, xlabel=r"Energy [eV]", ylabel=r"$I(\omega)$", xlim=[2.1,2.3])


axs[0].set(xlim=[2.175, 2.20])
axs[1].set(xlim=[2.175, 2.20])
axs[2].set(xlim=[2.15, 2.22])
axs[3].set(xlim=[2.14, 2.24])

fig.tight_layout()
fig.savefig("Classical_NEW_FFT.png")
