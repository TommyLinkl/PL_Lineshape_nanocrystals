...
Temp_range = np.array([1, 2])

linewidthQM_range = np.zeros(0)
freqShiftQM_range = np.zeros(0)

k = open("noCoupling_WidthFreqShift_vs_T.dat", "w")
...
for T in Temp_range: 
    ...
    I_file_QM = "noCoupling_Spectrum_T=%d.dat" % (T)
    F_file_QM = "noCoupling_F_T=%d.dat" % (T)
    ...
    (spectrum_w, spectrum_I, linewidth_QM_E17, freq_shift_QM_E17, F_t_Re_QM_E17, F_t_Im_QM_E17, F_t_Re_erf, F_t_Im_erf, DC_t_Re, DC_t_Im) = calc_spectrum_QM_E17_noCoupling(T, t_range, w_range, cutoff, slope, exc_E, phonon_filename, coupling_filename, I_file_QM, FWHM_file)
